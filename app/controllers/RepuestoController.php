...
require_once ROOT . '/app/models/RepuestoModel.php';
...
class RepuestoController {
    ...
    public function index(): void {
        ...
        $datos = [
            ...
            'repuestos'      => $this->model->getAll($cuenta_id, $veterinaria_id),
        ...
        $this->render('repuestos/index', $datos);
    }
    ...
    // Sube la foto del repuesto, la comprime a una miniatura liviana y la guarda en
    // assets/img/repuestos/{negocio}/. Retorna la ruta relativa guardada en BD
    // (ej. "mi_negocio/rep_abc123.jpg"), null si no se envió ninguna, o false si
    ...
    private function procesarImagen(?array $file, int $cuenta_id): string|false|null {
        ...
        $destinoDir = ROOT . '/assets/img/repuestos/' . $carpeta;
    ...
    public function transferir(): void {
        ...
        foreach ($ids as $i => $rid) {
            ...
            if ($rid > 0 && $cant > 0) {
                $lineas[] = ['producto_id' => $rid, 'cantidad' => $cant,
                             'precio_unitario' => 0, 'subtotal' => 0];
            }
        }

        if (empty($lineas)) {
            $_SESSION['flash_error'] = 'Agrega al menos un repuesto con cantidad mayor a 0.';
            $this->redirect('repuestos');
        }

        $ingresoModel = new IngresoRepuestoModel();
    ...
    private function validarFormulario(array $post): array {
        ...
        if (empty($datos['nombre'])) {
            $datos['error'] = 'El nombre del repuesto es obligatorio.';
            ...
            $datos['error'] = 'Ese código de barra ya está asignado a otro repuesto.';
        }
